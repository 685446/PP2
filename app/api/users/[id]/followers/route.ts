...
export async function GET(
  ...
  try {
    ...
    const followerIds = followers.map((entry) => entry.follower.id);
    const viewerFollowedIds = viewerUserId && followerIds.length > 0
      ? new Set(
          (
    ...
    return NextResponse.json({
      followers: followers.map((f) => ({
        ...
        isFollowing: viewerFollowedIds.has(f.follower.id),
      })),
      total,
      page,
      limit,
